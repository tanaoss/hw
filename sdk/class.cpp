...
bool checkIsTrySeparate(int robotA_id, int robotB_id)
{
    ...
    pair<double, double> next_posA = getNextPos(robotA_id);
    pair<double, double> next_posB = getNextPos(robotB_id);
    if ((state.FrameID >= 6571 && state.FrameID <= 6580) && robotA_id == 1 && robotB_id == 2)
    {
...
void control(vector<PayLoad> payLoad)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        // int can_st_flag=1;
        double stop_dis = (robots[i].xy_pos.first * robots[i].xy_pos.first + robots[i].xy_pos.second * robots[i].xy_pos.second) / (2 * payLoad[i].acceleration);
...
pair<int, double> pick_point(int robot_id, int state)
{
    ...
    if (state == 1)
    {
        for (i = 0; i <= studios.size(); i++)
        {
            ...
            { // 123 and no robot choose ,first choose ,get
                // cerr<<i<<endl;
    ...
    else if (state == 2)
    {
        for (i = 0; i < studios.size(); i++)
        {
            if (studios[i].type >= 1 && studios[i].type <= 3 && studios[i].r_id == -1 && studios[i].pStatus == 1)
            { // 123 and no robot choose ,get
    ...
    else if (state == 3)
    {
        for (i = 0; i < studios.size(); i++)
        {
            if (studios[i].type >= 4 && studios[i].type <= 6 && studios[i].r_id == -1 && studios[i].pStatus == 1)
            { // 456 and no robot choose ,get
                if (material[studios[i].type].size() > 0 && robot_get_type[studios[i].type] < material[studios[i].type].size())
                {
    ...
    else if (state == 4)
    {
        for (i = 0; i < studios.size(); i++)
        {
            if (studios[i].type == 7 && studios[i].r_id == -1 && studios[i].pStatus == 1)
    ...
    { // send
        if (item_type == 1)
        {
            for (i = 0; i < studios.size(); i++)
            {
                if (((studios[i].type == 4 || studios[i].type == 5) && (studios_rid[i][item_type] == -1) && (studios[i].bitSatus & 2) == 0) || studios[i].type == 9)
                { // 1 and no robot choose ,send
                    dist = calcuDis(robots[robot_id].pos, studios[i].pos);
                    if (dist < min)
                    {
                        min = dist;
        ...
        else if (item_type == 2)
        {
            for (i = 0; i < studios.size(); i++)
            {
                if (((studios[i].type == 4 || studios[i].type == 6) && (studios_rid[i][item_type] == -1) && (studios[i].bitSatus & 4) == 0) || studios[i].type == 9)
                { // 2 and no robot choose ,send
                    dist = calcuDis(robots[robot_id].pos, studios[i].pos);
                    if (dist < min)
                    {
                        min = dist;
        ...
        else if (item_type == 3)
        {
            for (i = 0; i < studios.size(); i++)
            {
                if (((studios[i].type == 5 || studios[i].type == 6) && (studios_rid[i][item_type] == -1) && (studios[i].bitSatus & 8) == 0) || studios[i].type == 9)
                { // 3 and no robot choose ,send
                    dist = calcuDis(robots[robot_id].pos, studios[i].pos);
                    if (dist < min)
                    {
                        min = dist;
        ...
        else if (item_type == 4)
        {
            for (i = 0; i < studios.size(); i++)
            {
                if ((studios[i].type == 7 && (studios_rid[i][item_type] == -1) && (studios[i].bitSatus & 16) == 0) || studios[i].type == 9)
                { // 4 and no robot choose ,send
                    dist = calcuDis(robots[robot_id].pos, studios[i].pos);
                    if (dist < min)
                    {
                        min = dist;
        ...
        else if (item_type == 5)
        {
            for (i = 0; i < studios.size(); i++)
            {
                if ((studios[i].type == 7 && (studios_rid[i][item_type] == -1) && (studios[i].bitSatus & 32) == 0) || studios[i].type == 9)
                { // 5 and no robot choose ,send
                    dist = calcuDis(robots[robot_id].pos, studios[i].pos);
                    if (dist < min)
                    {
                        min = dist;
        ...
        else if (item_type == 6)
        {
            for (i = 0; i < studios.size(); i++)
            {
                if (((studios[i].type == 7) && (studios_rid[i][item_type] == -1) && (studios[i].bitSatus & 64) == 0) || studios[i].type == 9)
                { // 6 and no robot choose ,send
                    dist = calcuDis(robots[robot_id].pos, studios[i].pos);
                    if (dist < min)
                    {
                        min = dist;
        ...
        else if (item_type == 7)
        {
            for (i = 0; i < studios.size(); i++)
            {
                if ((studios[i].type == 8 || studios[i].type == 9))
                { // 7 and no robot choose ,send
                    dist = calcuDis(robots[robot_id].pos, studios[i].pos);
                    if (dist < min)
                    {
                        min = dist;
    ...
    for (i = 0; i < robots.size(); i++)
    {
        ...
        if (robots[i].loc_id == robots[i].target_id && robots[i].target_id != -1)
        {
            if (robots[i].get_type == 0)
            {
                ...
                {
                    if (!checkeTimeEnough(i, robots[i].target_id, 9000 - state.FrameID))
                        ins[i].buy = -1;
                }
            }
            else
...
void robot_action()
{
    ...
    int full = 0;
    if (judge_full(2, 0.1))
        full = 1; // 4,5,6 full threshold
    if (judge_full(3, 0.2))
        full = 2; // 7 full threshold Higher priority
    // cerr<<" full = "<<full<<endl;
