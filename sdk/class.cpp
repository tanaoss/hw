            ...
            //  cerr<<"aaa1"<<endl;
                ...
                // if(studios[full_product[i]].pStatus == 1){
                    ...
                    // dist= precise_distance(robot_id,full_product[i])+wait_dis(robot_id,full_product[i])+target_obstacle_avoidance(robot_id,full_product[i]);
                        ...
                        if(class_map == 3){
                            if (check_area(robot_id, full_product[i]))
                            {
                                dist = distance(robot_id, full_product[i]).first + wait_dis(robot_id, full_product[i]);
                                if (dist < min)
                                {
            ...
            if(studios[i].type >= 1 && studios[i].type <= 3 && (studios[i].r_id==-1 ) ){  //123 and no robot choose ,get
                ...
                // {
                    ...
                    if(robot_get_type[studios[i].type]< material[studios[i].type].size()){
                        if (class_map == 3)
                        {
                            if (check_area(robot_id, i) && i!= 7 &&i!=21 &&i!=20 && i!=39 && i!=35)
                            {
            ...
            if(studios[i].type >= 4 && studios[i].type <= 6 && (studios[i].r_id==-1 )){  //456 and no robot choose ,get
                ...
                // {
                    ...
                    // if(robot_get_type[studios[i].type]< material[studios[i].type].size()){
                        if (class_map == 3)
                        {
                            if (check_area(robot_id, i))
                            {
                                dist = (distance(robot_id, i).first + wait_dis(robot_id, i)) * calc_priority(i)*0.5;
                                if (dist < min)
                                {
                                    // cerr << "dist = " << dist << " studio = " << i << endl;
                                    min = dist;
                    ...
                    if(studios[i].type == material_send[item_type][j] && (studios_rid[i][item_type] == -1) ){
                        if(((studios[i].bitSatus & ((int)pow(2,item_type)))==0)||((check_material_full(i)&&(studios[i].pStatus != 1)&&(studios[i].r_time>0&&(checkEnough(robot_id,i,studios[i].r_time)))))){
                            ...
                            if (class_map == 3)
                            {
                                if (check_area(robot_id, i))
                                {
                                    dist = (distance(robot_id, i).first * studio_wait_time(robot_id, i)) * change_area(robot_id, i);
                                    // cerr << calc_priority(i) << ' ' << (distance(robot_id, i).first * studio_wait_time(i)) << endl;
                                    // dist=calcuDis(robots[robot_id].pos,studios[i].pos);
                                    if (dist < min)
                                    {
                                        // cerr << "dist = " << dist << " studio = " << i << endl;
                            ...
                            {
                                dist = (distance(robot_id, i).first * studio_wait_time(robot_id, i)) * (calc_priority(i));
                                // cerr << calc_priority(i) << ' ' << (distance(robot_id, i).first * studio_wait_time(i)) << endl;
                                // dist=calcuDis(robots[robot_id].pos,studios[i].pos);
                                if (dist < min)
                                {
                ...
                if (studios[i].pStatus == 1 || (studios[i].r_time > 0))
                {
                    ...
                    if (robot_get_type[studios[i].type] < material[studios[i].type].size())
                    {
                        if (class_map == 3)
                        {
                            if (check_area(robot_id, i))
                            {
                                dist = distance(robot_id, i).first + wait_dis(robot_id, i) + back_dis(i);
...
Ins contr_one_rob(const Robot& robot , const PayLoad& payload){
    ...
    // }
        ...
        if(lt(dis,(getRobotRadius(robot.id)+sle_dis))&&!can_st){
            if(contr_print_flag&&state.FrameID>=6469&&state.FrameID<=6479&&robot.id==2){
                cerr<<"contr bug"<<endl;
        ...
        if(isNearWall(robot.id)&&isWall(robot.id)&&lt(dis,1)){
...
void collision_solve(int frame){
    ...
    for(i = 0; i < 2; ++i) {
        ...
        if(cerr_falg) {
            cerr<<ro[choose_id].id<<"target:"<<ro[choose_id].target_id<<" angle:"<<payloads[ro[choose_id].id].angle<<endl;
            cerr<<ro[x].id<<"target:"<<ro[x].target_id<<" angle:"<<payloads[ro[x].id].angle<<endl;
        ...
        if(ans != -1) {
            ...
            if(last_solution[ro[choose_id].id][ro[x].id] != -1) {
                ...
                updateIns(ro[choose_id].id, last_solution[ro[choose_id].id][ro[x].id]);
    ...
    // int stopID, goID;
    // double new_speed, relative_speed_sum;
    // double relative_dis;
    // pair<double, double> relative_speed;
    // for(i = 0; i < 4; ++i) {
    //     for(j = i+1; j < 4; ++j){
    //         if(robots[i].target_id == robots[j].target_id&& robots[j].target_id!=-1) {
    //             stopID = gt(payloads[i].distance, payloads[j].distance)? i: j;
    //             goID = (stopID == i)? j: i;
                

    //             // relative_dis = calcuDis(robots[i].pos, robots[j].pos);
    //             // relative_speed = subVector(robots[stopID].xy_pos, robots[goID].xy_pos);
    //             // relative_speed_sum = calVectorSize(relative_speed) * (gt(calVectorProduct(relative_speed, robots[stopID].xy_pos), 0)? 1: -1);
    //             // if(lt(relative_speed_sum, 0)) continue;

    //             if(!will_collision(i,j)) continue;

    //             cerr<<"time:"<<state.FrameID<<endl<<"target"<<robots[j].target_id<<endl;
    //             cerr<<stopID<<"**"<<payloads[stopID].speed<<endl;
    //             cerr<<goID<<"**"<<payloads[goID].speed<<endl;
    //             // cerr<<"relative_speed"<<relative_speed_sum<<endl;
    //             cerr<<"before:"<<ins[stopID].forward<<endl;
    //             // if(eq(relative_speed_sum, 0)) {
    //             //     cerr<<gt((relative_dis / 6), 0.4 * Pi)<<endl;
    //             //     ins[stopID].forward = gt((relative_dis / 6), 0.4 * Pi)? ins[stopID].forward: payloads[stopID].speed * 0.8;
    //             // }
    //             // else {
    //             //     cerr<<gt((relative_dis / 6), 0.04 * Pi)<<endl;
    //             //     ins[stopID].forward = gt((relative_dis / 6), 0.4 * Pi)? ins[stopID].forward: payloads[stopID].speed * 0.8;
    //             // }
                
    //             new_speed = payloads[stopID].distance / (payloads[goID].distance / payloads[goID].speed + 1);
    //             ins[stopID].forward = min(new_speed, ins[stopID].forward);
    //             cerr<<"after:"<<ins[stopID].forward<<endl;
    //         }
    //     }
    // }
