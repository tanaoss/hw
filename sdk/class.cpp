...
{
    double dis = dis_to_studios[target_id][1][robots[robot_id].close_node]+0.4;
    if (dis > 0)
    {
...
//     }
    ...
    //     cerr<<"ins:"<<ins[1].forward<<"  "<<ins[1].rotate<<"\n";
    // }
    ...
    //  if(state.FrameID == 6653)cerr << "start collision_solve \n";
     collision_solve(25);
    //  if (state.FrameID == 6653)
    //     cerr << "end \n";
    ...
    if(studios[studio_id].type>3){
        for(i = 1;i<= studio_material[studios[studio_id].type-4][0];i++){
            ...
            if(studios_rid[studio_id][i]!=-1){
                if(robots[studios_rid[studio_id][i]].get_type != 0)
                break;
            }
        }
        ...
        if(studios[studio_id].r_time != 0 ){
            ...
            for (int i = 1; i <= studio_material[studios[studio_id].type - 4][0]; i++)
            {
                ...
                if(robot != -1){
                    if(robots[robot].get_type!=0){
                        dist = dis_to_studios[studio_id][1][robots[robot].close_node];
                    }
                    else{
                         if(state.FrameID>start_time&&state.FrameID<end_time&&cerr_flag_j){
                        ...
                        if(robots[robot].target_id != -1)    
                            dist = dis_to_studios[robots[robot].target_id][0][robots[robot].close_node]+dis_to_studios[studio_id][1][studios[robots[robot].target_id].node_id];
                        else{
            ...
            if(studios[studio_id].r_time==-1){
                ...
                if(gt((dist/6/0.02),studios[studio_id].r_time)){
                    wait_dist = 0;
                    if(gt(studios[studio_id].r_time*0.02*6-max_dist,0)){
                        wait_dist = studios[studio_id].r_time*0.02*6-max_dist;
                    }
                    if (gt(dis, max_dist + wait_dist))
                    {
                        // cerr<<"true2\n";
                        if(state.FrameID>start_time&&state.FrameID<end_time&&cerr_flag_j){
                            cerr<<"true"<<endl;
...
bool check_robots_wait_closest(int robot_id, double dist_robot,int studio_id){
    ...
    for(int i=0;i<robots.size();i++){
        if(i!=robot_id&&robots[i].target_id != -1){
            if(robots[i].get_type == 0){
                dist = dis_to_studios[studio_id][0][robots[i].close_node]; 
            }
            else{
                dist = dis_to_studios[robots[i].target_id][1][robots[i].close_node];
...
bool check_robot_close(int robot_id,int studio_id){
    ...
    double dist_robot = dis_to_studios[studio_id][0][robots[robot_id].close_node];
    ...
    for(int i = 0;i<robots.size();i++){
        if(i!=robot_id &&robots[i].target_id != -1){
            if(robots[i].get_type != 0){
                ...
                dist = dis_to_studios[studio_id][1][robots[i].close_node];
            }
            if(lt(dist,dist_robot))count++;
        }
...
double check_suspicious_dis(int robot_id,int studio_id,int send_id,double dis){
    ...
    double dist;
    int min_subscript = -1;
    if(lt(dis_to_studios[studio_id][1][robots[robot_id].close_node],dis_to_studios[send_id][1][robots[robot_id].close_node])){
        return dis;
    }
    ...
    if(state_type ==2){
        for(int i =0;i<studios.size();i++){
            if(studios[i].type>3){
                for(int j = 1;j<=studio_material[studios[i].type-4][0];j++){
                    if(state.FrameID>start_time&&state.FrameID<end_time&&cerr_flag_j){
                            cerr << "studio : " << i << "material_type : " << studio_material[studios[i].type - 4][j] << " material rid : " << studios_rid[i][studio_material[studios[i].type - 4][j]] << " has " << (studios[i].bitSatus & (int)pow(2, studio_material[studios[i].type - 4][j])) << endl;
                        }
                    if(((studios[i].bitSatus & (int)pow(2,studio_material[studios[i].type-4][j])) == 0||(check_material_full(i)&&studios[i].r_time == -1)) && (studios[i].type>7||studios_rid[i][studio_material[studios[i].type-4][j]]==-1)){
                        ...
                        for(int k=0;k<studios[i].material_studios[j-1].size();k++){
                            ...
                            if((studios[material_studio_id].pStatus == 1||(studios[material_studio_id].r_time>0)) && studios[material_studio_id].r_id < 50 ){         
                                ...
                                dist = dis_to_studios[material_studio_id][0][robots[robot_id].close_node];   //
                                time = dist/6/0.02;
                                // cerr << "dits = "<<dist << endl;
                                ...
                                if(studios[material_studio_id].r_id != -1){
                                    if(studios[material_studio_id].type>3){
                                        ...
                                        dist2 = dist-dis_to_studios[material_studio_id][0][robots[studios[material_studio_id].r_id ].close_node];
                                ...
                                if(state.FrameID>start_time&&state.FrameID<end_time&&cerr_flag_j){
                                    cerr<<"to buy dist = "<< dis_to_studios[material_studio_id][0][robots[robot_id].close_node]<<" to send dist = "<<dis_to_studios[i][1][studios[material_studio_id].node_id]<<"\n";
                ...
                if(studios[i].type>3){
                    for(int j = 1;j<=studio_material[studios[i].type-4][0];j++){
                        ...
                        if(((studios[i].bitSatus & (int)pow(2,studio_material[studios[i].type-4][j])) == 0||(check_material_full(i)&&studios[i].r_time == -1))){
                            if (state.FrameID > start_time && state.FrameID < end_time && cerr_flag_j)
                            {
                                cerr << "material " << i << studio_material[studios[i].type - 4][j] << "has or not " << (studios[i].bitSatus & (int)pow(2, studio_material[studios[i].type - 4][j])) << "full_stuidio ?  " << check_material_full(i) << endl;
                            }
                            for(int k=0;k<studios[i].material_studios[j-1].size();k++){
                                material_studio_id = studios[i].material_studios[j-1][k];
                ...
                if(item_type != 7){
                    if(studios[i].type == material_send[item_type][j] && (studios_rid[i][item_type] == -1) ){
                        if(((studios[i].bitSatus & ((int)pow(2,item_type)))==0)||((check_material_full(i)&&(studios[i].pStatus != 1)&&(studios[i].r_time>0&&(checkEnough(robot_id,i,studios[i].r_time)))))){
                            dist = dis_to_studios[i][1][robots[robot_id].close_node];   //修改
                            time = dist/4/0.02;
                            if(eq(dist,10000))continue;
                            if(flag == 1){
                                if(time>(15000-state.FrameID))continue;
                            ...
                            income_ratio = (income/dist);
                            if(state.FrameID>start_time&&state.FrameID<end_time&&cerr_flag_j){
                                    cerr<<" to send dist = "<<dis_to_studios[i][1][robots[robot_id].close_node]<<"\n";
                    ...
                    if(studios[i].type == material_send[item_type][j]){
                        dist = dis_to_studios[i][1][robots[robot_id].close_node];
                        if(eq(dist,10000))continue;
                        income = price[item_type][1]-price[item_type][0];
                        income = income + (price[studios[i].type][1]-price[studios[i].type][0])/(studio_material[studios[i].type-4][0]*2)*(check_lack(i));
...
bool check_robots_change_closest(int robot_id, pair<pair<int,int>,double>temp){
    ...
    for(int i=0;i<robots.size();i++){
        if(i!=robot_id){
            if(robots[i].get_type == 0){
                dist = dis_to_studios[temp.first.first][0][robots[i].close_node]; 
            }
            else{
                dist = dis_to_studios[robots[i].target_id][1][robots[i].close_node];
    ...
    for(int i=0;i<4;i++){
        if(robots[i].loc_id == robots[i].target_id && robots[i].target_id != -1){
            if(robots[i].get_type != 0){
                ...
                robots[i].get_type = 0;
                complete_trans(i,0);
                robots[i].cnt_tar=robots[i].close_node;
            }
            else{
                //do something buy;
                // cerr<<"bbb\n";
...
void new_first_action(){
    ...
    {
        last_message[i][0] = 0;
        last_message[i][1] = robots[i].target_id;
        last_message[i][2] = robots[i].target_id_buy;
        last_message[i][3] = robots[i].target_id_send;
    }
...
void new_robot_action(){
    ...
    for(int i = 0;i<4;i++){
        // cerr<<"robot : "<<i<<" get_type = "<<robots[i].get_type<<"last hope = "<<last_message[i][0]<<endl;
        if(robots[i].get_type != last_message[i][0]){
            if(last_message[i][0] == 0){
                if(studios[robots[i].target_id_send].type!=8&&studios[robots[i].target_id_send].type!=9)studios_rid[robots[i].target_id_send][studios[robots[i].target_id_buy].type] = -1;
                if (studios[robots[i].target_id].r_id >= 50)
                    studios[robots[i].target_id].r_id -= 50;
                else
                    studios[robots[i].target_id].r_id = -1;
                robots[i].target_id = last_message[i][1];
                robots[i].target_id_buy = last_message[i][2];
                robots[i].target_id_send = last_message[i][3];
                robots[i].cnt_tar = robots[i].close_node;
                if(studios[robots[i].target_id_send].type!=8&&studios[robots[i].target_id_send].type!=9)studios_rid[robots[i].target_id_send][studios[robots[i].target_id_buy].type] = i;
            }
            else{
                if (studios[robots[i].target_id].r_id >= 0)
                    studios[robots[i].target_id].r_id += 50;
                else
                    studios[robots[i].target_id].r_id = i;
                robots[i].target_id = last_message[i][1];
    ...
    for(int i = 0;i<robots.size();i++){
        if (robots[i].real_get_type == last_message[i][0]){
            last_message[i][1] = robots[i].target_id;
            last_message[i][2] = robots[i].target_id_buy;
            last_message[i][3] = robots[i].target_id_send;
        }
    ...
    for(int i = 0;i<robots.size();i++){
        if (robots[i].real_get_type == last_message[i][0]){
            last_message[i][0] = robots[i].get_type;
        }
    }
    // for(int i = 0;i<4;i++){
...
vector<pair<double,double>>Calculate_the_trajectory(Robot rob,Ins ins_in, int forward_change, int rotate_change,const vector<pair<double,double>>&  tra,int cnt,int tar,double rob_dis,double pre_dis){
    ...
    for( cnt=0;(cnt<=tar&&cnt<tra.size());cnt++){
        ...
        if(lt(tmpDis,rob_dis)){
            new_cllo_time=cnt*0.02;
            if(state.FrameID==11459&&(rob.id==1||rob.id==2))
            cerr<<"---rob collossi----"<<endl;
            return {};
        }
            // 撞障碍物，返回空
        if(checkNearBar(rob.pos, rob.radius)){
            // printPair(rob.pos);
            // cerr<<"撞障碍物\n";
              if(state.FrameID==11448&&(rob.id==1||rob.id==2))
            cerr<<"----bar collossi----"<<endl;
            return {};
        }
        double seta=rob.direction;
        double w=rob.angular_velocity;
...
// cerr<<"rob 2"<<endl;
   ...
   bool print_cerr_flag_ta1=false;
    ...
    // if( state.FrameID>6501&&state.FrameID<6527){
    //     // cerr<<robot.need_adjust_statues<<" rob "<<robot.id<<endl;
    //     print_cerr_flag_ta1=true;
    //     print_rob_id=1;
    // }
...
//    cerr<<"sha ye meigan "<<endl;
    ...
    if(lt(payload.distance,1)){
        if(studios[robot.target_id].r_time>=50||robot.get_type<=3)
            ins_t.forward=3;
        else if(studios[robot.target_id].r_time<50&&robot.get_type>3){
            ins_t.forward=2;
        }
    }else if(lt(payload.distance,0.5)){
...
void collision_solve(int frame){
    ...
    // if(state.FrameID >= 0 && state.FrameID <= 6000 && 999==999)
    ...
    for (i = 0; i < 4; i++)
    {
        for (j = i + 1; j < 4; j++)
        {
            ...
            if(tmp == 9000 && lt(calcuDis(ro[i].pos, ro[j].pos), 5)) {
                if(!check_node_safe(ro[i].pos, mindis, ro[j], {}) || !check_node_safe(ro[j].pos, mindis, ro[i], {})) {
                    tmp = -1;
                    coll_time[i][j] = tmp;
                    coll_time[j][i] = tmp;
                    if(collision_cerr_flag) {
                        cerr<<"time:"<<state.FrameID<<"\n";
                        cerr<<ro[i].id<<"-"<<ro[j].id<<"may collison"<<tmp<<"\n";
                        cerr<<"dis:"<<calcuDis(ro[i].pos, ro[j].pos)<<endl;
    ...
    for(i = 0; i < 2; ++i) {
        ...
        if(collision_cerr_flag) cerr<<"kkk"<<endl;
        ...
        if(!vis[x] && ro[choose_id].target_id != -1 && ro[x].target_id != -1) {
            //离目标台更远的躲避
        ...
        ins_num = 4;
        for(k = 0; k < ins_num && coll_time[choose_id][x] != -1; ++k) {
            if(k < 3) {
        ...
        if(ans != -1 && gt(payloads[ro[choose_id].id].speed, 1) && (!eq(ins[ro[choose_id].id].forward, 0) || ro[choose_id].target_id == -1 
                        || gt(calcuDis(ro[choose_id].pos, studios[ro[choose_id].target_id].pos), 1))
                        && (ans != 3|| !check_nead_slow_down(ro[x], ro[choose_id], mindis, coll_time[choose_id][x], trajectory[x]))) {
            ...
            if(vis[x] && lt(ins[ro[x].id].forward, 0)) {
                do_avoid(ro[choose_id], ro[x], mindis, trajectory[x]);
            }
            ...
            if(check_nead_slow_down(ro[choose_id], ro[x], mindis, coll_time[choose_id][x], trajectory[choose_id])) {
                if(collision_cerr_flag) {
                    cerr<<"choose avoid\n";
                }
                if (!do_avoid(ro[choose_id], ro[x], mindis, trajectory[x]))
                {
                    if (collision_cerr_flag)
                        cerr << "choose can not avoid\n";
                    flag = do_avoid(ro[x], ro[choose_id], mindis, trajectory[choose_id]);
                    if (!flag)
                        cerr << state.FrameID << " " << ro[choose_id].id << "&" << ro[x].id << " no way to avoid\n";
                }
            }
            else if(check_nead_slow_down(ro[x], ro[choose_id], mindis, coll_time[choose_id][x], trajectory[x])) {
                if(collision_cerr_flag) {
                    cerr<<"x avoid\n";
                }
                flag = do_avoid(ro[x], ro[choose_id], mindis, trajectory[choose_id]);
        ...
        if(collision_cerr_flag) {
            cerr<<"choose ins:"<<ins[ro[choose_id].id].forward<<" "<<ins[ro[choose_id].id].rotate<<"\n";
            cerr<<"x ins:"<<ins[ro[x].id].forward<<" "<<ins[ro[x].id].rotate<<"\n";
            cerr<<"------------\n";
        }
...
int get_avoid_node(const Robot &ro_back, const Robot &ro_go, double mindis, const vector<pair<double,double>> &trajectory) {
    ...
    while(!q.empty()) {
        ...
        if(cerr_flag) {
            cerr<<"node_id:"<<from<<" dis:"<<dis<<" pre_id:"<<pre_id<< "angle_sum:"<<now_node.angle_sum<<"\n";
        }
        if (check_node_safe(exist_id[is_take][from], mindis, ro_go, trajectory) && gt(calcuDis(exist_id[is_take][to], ro_back.pos), ro_back.radius))
        {
            to = from;
            if (collision_cerr_flag) {
                cerr << "safeto:" << to << "\n";
        ...
        for(i = 0; i < num; ++i) {
            ...
            if(cerr_flag) {
                cerr<<"to:"<<to<<" dis:"<<calcuDis(exist_id[is_take][to], ro_go.pos)<<" min:"<<mindis<<endl;
            }
...
bool do_avoid(const Robot &ro_stop, const Robot &ro_go, double mindis, const vector<pair<double,double>> &trajectory) {
    int to = get_avoid_node(ro_stop, ro_go, mindis, trajectory);
    if(to == -1) return false;

    pair<double, double> to_pos = exist_id[(ro_stop.get_type != 0)][to];
    PayLoad pay_back = calPayload_back(ro_stop, to_pos);
    PayLoad pay = calPayload(ro_stop, to_pos);
    double time, time_back;
    int id = ro_stop.id;
    int is_take = (ro_stop.get_type != 0);
    
    robots[id].cnt_tar = robots[id].node_id;


    time = get_rotation_stop_time(ro_stop, pay);
    ...
    if (!check_node_safe(exist_id[(ro_stop.get_type != 0)][ro_stop.close_node], mindis, ro_go, trajectory)) {
        if(collision_cerr_flag) cerr<<"减速\n";
        ins[ro_go.id].forward = min(1.5, ins[ro_go.id].forward);
    }
    // else if(collision_cerr_flag) {
    //     printPair(exist_id[(ro_stop.get_type != 0)][ro_stop.close_node]);
    //     printPair(ro_stop.pos);
    // }
    return true;
}


// bool do_back(int id, int id2, pair<double, double> pos) {
//     int to = choose_best_to(robots[id], pos);

//     if(state.FrameID >= 10400 && state.FrameID <= 12000) {
//         to = get_avoid_node(robots[id], robots[id2], robots[id].radius + robots[id2].radius);
//         cerr<<"!!!:"<<to<<endl;
//     }
...
//ro will collision ro_static.pos
bool check_nead_slow_down(const Robot &ro, const Robot &ro_static, double mindis, int coll_frame, const vector<pair<double,double>> &trajectory) {
    ...
    while(tar != -1 && next_node[tar][is_take][node1] != -1) {
        node1 = next_node[tar][is_take][node1];
        cnt++;
        if(lt(calcuDis(exist_id[is_take][node1], ro_static.pos), mindis))
            return true;
        
        if(index < num && le(calcuDis(ro_static.pos, trajectory[index++]), mindis))
            return true;
        // if(collision_cerr_flag) {
    ...
    if(cnt < coll_frame && is_take == 0 && tar != -1 && studios[tar].pStatus == 0 && studios[tar].r_time > coll_frame) {
        return true;
    }
...
bool check_node_safe(pair<double,double> pos, double mindis, const Robot &ro, const vector<pair<double,double>> &trajectory) {
    int tar = ro.target_id;
    int is_take = (ro.get_type != 0);
    int node1 = ro.close_node;
    int cnt, num;
    ...
    while(next_node[tar][is_take][node1] != -1) {
        pre_dis = dis;
        node1 = next_node[tar][is_take][node1];
        dis = calcuDis(pos, exist_id[is_take][node1]);
        if(le(dis, mindis))
            return false;
        // if(collision_cerr_flag) {
        //     cerr<<"node"<<node1<<" ddd:"<< dis <<"\n";
        //     cerr<<"next:"<<next_node[tar][is_take][node1]<<endl;
        // }
        if(gt(dis, pre_dis)) {
            // if(collision_cerr_flag) {
            //     cerr<<"pre_dis"<<pre_dis<<" dis:"<< dis <<"\n";
            // }
            break;
        }
        
        if(cnt < num && le(calcuDis(pos, trajectory[cnt++]), mindis))
            return false;
...
void Dijkstra(int studio_id, int is_take) {
    ...
    bool cerr_flag = false;
    // if(studio_id == 12 && is_take == 0) cerr_flag = true;
    ...
    // if(studio_id == 12 && is_take == 0) cerr<<"num:"<<num<<endl;
    for(i = 0; i < num; ++i) {
            ...
            for(int i1=i-1;i1<=i+1;i1++){
                ...
                for(int j1=j-1;j1<=j+1;j1++){
                    ...
                    if(need_ad1&&illegal_point[0][id]&&exist_id[0].count(tmpID)&&!illegal_point[0][tmpID]
                        ...
                        con1=true;
                        exist_id[0][id]=exist_id[0][tmpID];
                        // if(id==4244)cerr<<4244<<"->"<<tmpID<<endl;
                        // printPair()
